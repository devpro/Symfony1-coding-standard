...
class Symfony_Sniffs_Commenting_ClassCommentSniff extends PEAR_Sniffs_Commenting_FileCommentSniff
{
    ...
    // ----------------------------------------------------
    // PEAR_Sniffs_Commenting_FileCommentSniff functions

    /**
     * (non-PHPdoc)
     * @see PEAR_Sniffs_Commenting_FileCommentSniff::register()
     */
    ...
    /**
     * (non-PHPdoc)
     * @see PEAR_Sniffs_Commenting_FileCommentSniff::process()
     */
    ...
    /**
     * (non-PHPdoc)
     * @see PEAR_Sniffs_Commenting_FileCommentSniff::processVersion()
     */
    ...
    /**
     * (non-PHPdoc)
     * @see PEAR_Sniffs_Commenting_FileCommentSniff::processPackage()
     */
    protected function processPackage($errorPos)
    {
        $this->checkPackage($this->commentParser->getPackage(), $errorPos);
    }

    /**
     * (non-PHPdoc)
     * @see PEAR_Sniffs_Commenting_FileCommentSniff::processSubpackage()
     */
    protected function processSubpackage($errorPos)
    {
        $this->checkPackage($this->commentParser->getSubpackage(), $errorPos);
    }
     ...
     * @param int $package
     */
    protected function checkPackage($package, $errorPos, $isSub = false)
    {
        if ($package !== null) {
...
//                     $error     = ($isSub ? 'Subpackage' : 'Package').' name "%s" is not valid; consider "%s" instead';
//                     $validName = trim($newName, '_');
//                     $data      = array(
//                         $content,
//                         $validName,
//                     );
//                     $this->currentFile->addError($error, $errorPos, $isSub ? 'InvalidSubpackage' : 'InvalidPackage', $data);
//                   }
              } else {
                $error = ($isSub ? '@subpackage' : '@package').' tag must contain a name';
                $this->currentFile->addError($error, $errorPos, $isSub ? 'EmptySubpackage' : 'EmptyPackage');
              }
        }
    }
}
